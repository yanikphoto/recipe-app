...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe, onSelect, onDelete }) => (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-200" onClick={onSelect}>
        <div className="w-full h-40 relative">
            <StoredImage imageId={recipe.imageUrl} alt={recipe.title} className="absolute inset-0 w-full h-full object-cover" />
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    onDelete();
                }}
                className="absolute top-3 right-3 bg-white/80 rounded-full p-1.5 shadow-md hover:bg-white transition-colors backdrop-blur-sm"
